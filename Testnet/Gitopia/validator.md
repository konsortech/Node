...
```
Import Validator Pharse Wallet to Kepler
...
Transfer funds
```
gitopiad tx bank send $GITPIA_WALLET_ADDRESS <TO_GITOPIA_WALLET_ADDRESS> 1000000utlore
```
...
```
gitopiad tx distribution withdraw-rewards $GITPIA_VALOPER_ADDRESS --from=$WALLET --commission --chain-id=$GITOPIA_CHAIN_ID
```
